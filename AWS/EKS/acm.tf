...
#### Certificate for RestAPI

resource "aws_acm_certificate" "api_cert" {
  count =  var.enable_ssl && var.enable_dns ? 1 : 0 
  domain_name       = "api.${var.environment}.memphis.${var.hostedzonename}"
  validation_method = "DNS"
  tags = local.tags
...
resource "aws_route53_record" "api_r53_records" {
  for_each =  var.enable_ssl && var.enable_dns ? {
...
resource "aws_acm_certificate_validation" "api_cert_validation" {
  count = var.enable_ssl && var.enable_dns ? 1 : 0
  certificate_arn         = aws_acm_certificate.api_cert[count.index].arn
  validation_record_fqdns = [for record in aws_route53_record.api_r53_records : record.fqdn]
}